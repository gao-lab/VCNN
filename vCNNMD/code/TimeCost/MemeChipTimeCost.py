...
def MeMeChip(InputFile, outputRoot):
    """
    :param InputFile: fasta
    :return:
    """
    softwarePath = "/home/lijy/anaconda3/bin/meme-chip"
    outputDir = outputRoot+ "/meme-chip/"
    mkdir(outputDir)
    tmp_cmd = softwarePath + " "+ InputFile + " " + "-oc "+ outputDir  + " -meme-p 1"
    os.system(tmp_cmd)
...
if __name__ == '__main__':
    ...
    DataRoot = "/rd2/lijy/KDD/vCNNFinal/VCNNMD/chip-seqFa/"
    mkdir(outputRoot)
    ...
    for i in range(len(TestLenlist)):
        ...
        MeMeChip(InputFilePath, outputRoot+"/MeMeChip/"+fileName+"/")
        end = time.time()
        print("timecost:",end-start)
        TimeTest.append([dataNumlist[i], dataBplist[i], end-start])
    np.savetxt("../../result/TimeCost/timeCostMemeChip.txt", np.asarray(TimeTest))
