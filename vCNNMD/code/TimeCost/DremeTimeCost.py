...
def Dreme(InputFile, outputRoot):
    """
    调用meme
    :param InputFile: fasta格式的文件
    :return:
    """
    softwarePath = "/home/lijy/anaconda3/bin/dreme"
    outputDir = outputRoot + "/dreme/"
    mkdir(outputDir)
    tmp_cmd = softwarePath + " " + "-oc "+ outputDir + " -p "+ InputFile
    os.system(tmp_cmd)
...
if __name__ == '__main__':
    ...
    for i in range(len(TestLenlist)):
        ...
        Dreme(InputFilePath, outputRoot+"/dreme/"+fileName+"/")
        end = time.time()
        print("timecost:",end-start)
        TimeTest.append([dataNumlist[i], dataBplist[i], end-start])
    np.savetxt("../../result/TimeCost/timeCostDreme.txt", np.asarray(TimeTest))
