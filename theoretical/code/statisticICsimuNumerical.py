...
def Draw(dict,path,ker_size_list):
	...
	plt.xlabel("${P_{ideal}}$", fontsize = 20)
	plt.ylabel("${P_{real}}$", fontsize = 20)
	legend = plt.legend(title="kernel length",loc='center left',bbox_to_anchor=(1, 0.5), fontsize = 20)
	plt.setp(legend.get_title(), fontsize=20)
	plt.ylim(0, 1)
	plt.text(0, 0.9, name, fontsize=20)
...
def DrawRank(dict,path,ker_size_list):
	...
	plt.xlabel("${P_{ideal}}$", fontsize = 20)
	plt.ylabel("Rank of ${P_{real}}$", fontsize = 20)
	legend = plt.legend(title="kernel length",loc='center left',bbox_to_anchor=(1, 0.5), fontsize = 20)
	plt.setp(legend.get_title(), fontsize=20)
	plt.ylim(0, 10)
	plt.text(0, 9, name,fontsize=20)
	plt.yticks(range(9))
