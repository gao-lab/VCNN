...
class kerenl_recover(object):
    ...
    def get_kernel(self,p):
        ...
        self.model = None
        return
    # get the kernel path of the best auc
    def get_best_model_path(self):
        ...
        print("model_info: ",ret.replace(".pkl", ".checkpointer.hdf5").\
            replace("/Report_KernelNum-","/model_KernelNum-"))
        return ret.replace(".pkl", ".checkpointer.hdf5").\
            replace("/Report_KernelNum-","/model_KernelNum-")
    # get the kernel of the best model
    def get_best_model_kernel(self):
        p = self.get_best_model_path()
        self.get_kernel(p)
    ...
    def DB_recover_kernel(self):
        ...
        for ker_idx in range(self.kernel.shape[-1]):
            ...
            if self.if_save(ret):
                sp = self.output_root+str(ker_idx)+".txt"
                np.savetxt(sp,ret)
        return
...
def demo():
    ...
    data_info_lst = get_simu_data_info_lst(simu_model_result)
