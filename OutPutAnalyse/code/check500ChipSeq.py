...
def DrawBox(dataName, path, outputName,OutputPath):
    ...
    :return:
    """
    fig, axs = plt.subplots(3, 4, figsize=(3 * 4, 4 * 4), squeeze=False)
    NamelistOut = []
    mkdir(OutputPath)
    for i in range(len(dataName)):
        ...
        if pvalue>=0.1:
            pV = "P-value: "+ '{:.3e}'.format(pvalue)
        else:
            pV = "P-value: "+ '{:.3e}'.format(pvalue)
        # axs[r][c].set_title("P-value: "+ format(pvalue,".2e"), fontsize=5)
        y, h = np.max([Pddict["CNN"].max(),Pddict["vCNN"].max(),0.9]), 0.01
        axs[r][c].set_ylim([0.4,max([y,1.1])])
...
def get_data_info():
    file = open('../../TrainCode/ChIpSeqLess500/WorseKey.txt', 'r')
...
if __name__ == '__main__':
    ...
    SimulationResultRoot = "../../OutPutAnalyse/result/ChIPSeq500/"
    deepbind_data_root = "../../Data/ChIPSeqData/HDF5/"
    OutputPath = "../../OutPutAnalyse/ModelAUC/ChIPSeq500/"
    mkdir(OutputPath)
    ...
    for item in model_lst:
        ...
        mkdir(OutputPath)
        Df.to_csv(OutputPath+ item + "AUC.csv")
        for key in aucouttem.keys():
            np.savetxt(OutputPath + key +"_" + item + "_auc.txt", np.asarray(aucouttem[key]))
        AUCDifference[item] = aucouttem
    # dataName = list(range(27))

    dataName = list(set(dataName))
    Namelist = DrawBox(dataName, OutputPath,dataName,"../../OutPutAnalyse/ModelAUC/ChIPSeq500/AucDifference/")
    DrawErrorBar(AUCDifference, "../../OutPutAnalyse/ModelAUC/ChIPSeq500/AucDifference/")
