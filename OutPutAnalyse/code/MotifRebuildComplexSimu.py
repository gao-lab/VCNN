...
def GeneRateRecoverKerDict(data_root, resultPath,outputPath):
    ...
    def ModelNameGenerate(ExactModel,ModelTypeStr):
        if ModelTypeStr == "CNN":
            ...
            ExactModelNameTem = ExactModel.split("/")[-1].split(".")[0].split("_")
            ExactModelNameT1 = ExactModelNameTem[1] + "_" + ExactModelNameTem[2]  + "_" + ExactModelNameTem[3]+ "_" + ExactModelNameTem[4]
            ExactModelNameT2 = ExactModelNameT1.replace("-", "_")
            ExactModelReport = glob.glob(
                ExactModel.replace(ExactModel.split("/")[-1],
                                   "Report*" + ExactModelNameT1 + "*"))[0]
    ...
    for DataType in DataTypelist:
        ...
        for Modeltype in mode_lst:
            ...
            for ExactModel in ExactModellist:
                ...
                i=i+1
                if ExactModelName not in KernelsDict[DataTypeStr][ModelTypeStr]:
                    KernelsDict[DataTypeStr][ModelTypeStr][ExactModelName] = {}
                if not os.path.isfile(outputPath + DataTypeStr + "/" + ModelTypeStr + "/" + ExactModelName + "/Over.txt"):
                    ...
                    np.savetxt(outputPath + DataTypeStr + "/" + ModelTypeStr + "/" + ExactModelName + "/Over.txt", np.arange(3))
                else:
                    print("trained"+str(i))
        gc.collect()
...
def recover_ker(model, resultPath,modeltype,input_shape):
    ...
    def build_mask(kernel_shape, k_weights):
        ...
        def init_left(kernel_shape, k_weights):
            ...
            k_weights_tem_2d_left = np.arange(kernel_shape[0])  # shape[0] is the length
            k_weights_tem_2d_left = np.expand_dims(k_weights_tem_2d_left, 1)
            k_weights_tem_3d_left = np.repeat(k_weights_tem_2d_left, kernel_shape[2], axis=1) - k_weights[0, :,:]  # shape[2]是numbers
            k_weights_3d_left = np.expand_dims(k_weights_tem_3d_left, 1)
            return k_weights_3d_left

        def init_right(kernel_shape, k_weights):
            k_weights_tem_2d_right = np.arange(kernel_shape[0])
            k_weights_tem_2d_right = np.expand_dims(k_weights_tem_2d_right, 1)
            k_weights_tem_3d_right = -(
            np.repeat(k_weights_tem_2d_right, kernel_shape[2], axis=1) - k_weights[1, :, :])  # shape[2]是numbers
        ...
        # Generate the same mask_tem as the previous initial shape.
            ...
            for ExactModel in ExactModellist:
                ...
                for motifid in MotifID:
                    ...
                    if modelTem.split("/")[-1] not in BestKernel[DataSetType][motifid]:
                        BestKernel[DataSetType][motifid][modelTem.split("/")[-1]] = [-math.log(motifidALL['E-value'].min(),10)]
                    else:
                        BestKernel[DataSetType][motifid][modelTem.split("/")[-1]].append(-math.log(motifidALL['E-value'].min(),10))
...
def TomTomFileFormatMotifGenerate(PwmsPath="/home/lijy/VCNNMore/SimulationFinal/Data/motifForSimu/realMotif"):
    """
    load all txt file ，save in the meme formate
    :param PwmsPath:
    :return:
    """
...
if __name__ == '__main__':
    SimulationDataRoot = "../../Data/MoreComplexSituation/HDF5/"
    SimulationResultRoot = "../../OutPutAnalyse/result/MoreComplexSituation/"
    outputPath = "../../OutPutAnalyse/MotifRebuild/MoreComplexSituation/"
