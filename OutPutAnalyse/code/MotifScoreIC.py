...
def score_ker_mtf(ker_cut_threshold = 0.01,result_root = "simu_kerDB_root",img_save_root = "ker_mtf_score_img_dir"):
    ...
    def draw_box_plot(data_info,ret,score_type):
        ...
        if num_of_mtf == 2:
            ...
            plt.subplot(121)
            plt.ylim(0,11)
            sns.boxplot(data=plot_data[mtf_name_lst[0]])
            plt.ylabel("similarity score")
            plt.xticks(np.arange(2),mode_lst)
            ...
            plt.subplot(122)
            plt.ylim(0,11)
            sns.boxplot(data=plot_data[mtf_name_lst[1]])
            plt.ylabel("similarity score")
            plt.xticks(np.arange(2),mode_lst)
            plt.title(" ".join([data_info,mtf_name_lst[1]]))
            #ax2.ylim(0,11)
        plt.savefig(save_p.replace(".png",".eps"))
        plt.savefig(save_p)
        return
...
def get_top_per5_model_path(p):
    ...
    r = [it[0].replace(".pkl", ".checkpointer.hdf5").replace("Report_KernelNum-","model_KernelNum-") \
        for it in sorted_list[-re_num:] ]
    return r

def SelectTopModel(modelNamelist):
    """
    ...
    for name in modelNamelist:
        AUC = int(name.split("/")[-3].split("_")[-1])
        ModelDict[name] = AUC
    re_num = int(len(ModelDict)*0.05) # the number of return list
    TopModellist = sorted_by_value(ModelDict)[-re_num:]
...
def NameTransForm(name):
    ...
    :return:
    """
    nameOutlist = name.split("/")[-3].split("_")
    NameOut = "model_KernelNum-"+ nameOutlist[1] \
              + "_initKernelLen-"+ nameOutlist[3] +"_maxKernelLen-40_seed-"+ nameOutlist[5] + "_batch_size-100"

    return NameOut

if __name__ == "__main__":
    root = "../../"
    mtf_root = root + "/Data/ICSimulation/RealMotif/"
    auc_box_dir = root + "result/auc_box/"
    simu_result_root = root + "result/simu_result/"
    simu_kerDB_root = root + "/OutPutAnalyse/MotifRebuild/ICSimulation/"
    ker_mtf_score_img_dir = root + "/OutPutAnalyse/MotifScore/ICSimulation/"

    ####### Generate Top5 model list ##########
