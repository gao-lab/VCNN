...
def DrawBox(dataName, path):
    ...
    for dataInfo in dataName:
        ...
        for filename in temfilelist:
            labels.append(filename.split("/")[-1].split("_")[2])
...
if __name__ == '__main__':
    ...
    for item in model_lst:
        ...
        mkdir("../../OutPutAnalyse/ModelAUC/ICSimulation/")
        Df.to_csv("../../OutPutAnalyse/ModelAUC/ICSimulation/"+ item + "AUC.csv")
        for key in aucouttem.keys():
            print(key)
            np.savetxt("../../OutPutAnalyse/ModelAUC/ICSimulation/" + key +"_" + item + "_auc.txt", np.asarray(aucouttem[key]))
    ...
    DrawBox(dataName, "../../OutPutAnalyse/ModelAUC/ICSimulation/")
