...
def score_ker_mtf(ker_cut_threshold = 0.01,result_root = "simu_kerDB_root",img_save_root = "ker_mtf_score_img_dir"):
    ...
    def draw_box_plot(data_info,ret,score_type):
        ...
        if num_of_mtf == 2:
            ...
            plt.xlabel(data_info)
            plt.xticks(np.arange(2),mode_lst)
            plt.title(mtf_name_lst[0])
            ...
            plt.xlabel(data_info)
            plt.xticks(np.arange(2),mode_lst)
            plt.title(mtf_name_lst[1])
            #ax2.ylim(0,11)

        plt.savefig(save_p)
        plt.savefig(save_p.replace(".png",".eps"), format="eps")
        return
...
if __name__ == "__main__":
    root = "../../"
    mtf_root = root + "/Data/SimulationOnTwoMotif/RealMotif/"
    auc_box_dir = root + "result/auc_box/"
    simu_result_root = root + "result/simu_result/"
    simu_kerDB_root = root + "/OutPutAnalyse/MotifRebuild/SimulationOnTwoMotif/"
    ker_mtf_score_img_dir = root + "/OutPutAnalyse/MotifScore/SimulationOnTwoMotif/"
